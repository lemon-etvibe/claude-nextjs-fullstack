...
> ⚠️ **중요**: 모든 작업 브랜치는 `dev`에서 분리해야 합니다.
> `main`에서 직접 분리하면 PR 타겟이 `main`이 되어 워크플로우가 꼬입니다.

### 브랜치 분리 및 PR 규칙

| 작업 | 기반 브랜치 | PR 타겟 |
|------|------------|---------|
| 기능 개발 | `dev` → `feature/*` | `feature/*` → `dev` |
| 버그 수정 | `dev` → `fix/*` | `fix/*` → `dev` |
| 릴리스 | `dev` | `dev` → `main` |
| 핫픽스 (긴급) | `main` → `hotfix/*` | `hotfix/*` → `main` |
