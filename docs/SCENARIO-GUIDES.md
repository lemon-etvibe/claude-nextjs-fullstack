...
# 1. 스키마 설계 리뷰
> /enf:schema-design
# 현재 campaigns 테이블에 budget 필드 추가 계획 설명

# 2. 스키마 수정 요청
> src/db/schema.ts의 campaigns 테이블에 budget 필드를 추가해줘
# 타입: numeric, 기본값: '0'
...
#### 주의사항
- `references(() => table.id, { onDelete: 'cascade' })` 사용 시 연쇄 삭제 주의
- 인덱스 필요 여부 검토
- 기존 데이터 마이그레이션 계획
...
| N+1 쿼리 | Drizzle with 사용 |
| 큰 번들 | dynamic import |
| 불필요한 데이터 | columns로 필요 필드만 |
...
> @dev-assistant 위 Handoff Artifact 기반으로 구현해줘. 구현 순서 1번(Drizzle 스키마)부터.
